node ('centos8') {
    ...
    stage('checkout') {
        checkout([$class: 'GitSCM',
            ...
            submoduleCfg: [],
            userRemoteConfigs: [[url: 'https://github.com/EuPathDB-Infra/docker-galaxy-python-tools.git']]
        ]
    )
    ...
    stage('build') {
      withCredentials([usernameColonPassword(credentialsId: '0f11d4d1-6557-423c-b5ae-693cc87f7b4b', variable: 'HUB_LOGIN')]) {
        ...
        // build the image
        sh 'podman build --format=docker -t galaxy-python-tools .'

        // push to dockerhub (for now)
        sh "podman push --creds \"$HUB_LOGIN\" galaxy-python-tools docker://docker.io/veupathdb/galaxy-python-tools:${tag}"
      }
